...
var Swiper = function(f, b) {
	...
	function v(c) {
		if (a.isTouched && !b.onlyExternal && !(u && "mousemove" == c.type)) {
			...
			if (!w)
				...
				else if (c.assignedToSwiper = !0, b.preventLinks && (a.allowLinks = !1), b.autoPlay && a.stopAutoPlay(), !u || 1 ==
				c.touches.length) {
				...
				if (b.resistance) {
					if (0 < a.positions.current && (!b.freeMode || b.freeModeFluid)) b.loop ? (c = 1, 0 < a.positions.current && (a.positions
							.current = 0)) : c = (2 * l - a.positions.current) / l / 2, a.positions.current = 0.5 > c ? l / 2 : a.positions
						.current * c;
					if (a.positions.current < -g() && (!b.freeMode || b.freeModeFluid)) b.loop ? (c = 1, d = a.positions.current, e = -
						g() - l) : (d = (a.touches.current - a.touches.start) * b.ratio + (g() + a.positions.start), c = (l + d) / l, d =
						a.positions.current - d * (1 - c) / 2, e = -g() - l / 2), a.positions.current = d < e || 0 >= c ? e : d
				}
				if (b.followFinger) {
					b.moveStartThreshold ? Math.abs(a.touches.current - a.touches.start) > b.moveStartThreshold || D ? (D = !0, h ? a
							.setTransform(a.positions.current, 0, 0) : a.setTransform(0, a.positions.current, 0)) : a.positions.current = a
						.positions.start : h ? a.setTransform(a.positions.current, 0, 0) : a.setTransform(0, a.positions.current, 0);
					b.freeMode && a.updateActiveSlide(a.positions.current);
					b.onSlideClick && a.clickedSlide && (a.clickedSlide = !1);
					b.grabCursor && (a.container.style.cursor = "move", a.container.style.cursor = "grabbing", a.container.style.cursor =
						"-moz-grabbin", a.container.style.cursor = "-webkit-grabbing");
	...
	function r() {
		if (!b.onlyExternal && a.isTouched) {
			...
			b.grabCursor && (a.container.style.cursor = "move", a.container.style.cursor = "grab", a.container.style.cursor =
			...
			0 < a.positions.current ? a.swipeReset() : a.positions.current < -e ? a.swipeReset() : b.freeMode ? (300 > a.times.end -
				a.times.start && b.freeModeFluid && (d = a.positions.current + 2 * a.touches.diff, d < -1 * e && (d = -e), 0 < d &&
					(d = 0), h ? a.setTransform(d, 0, 0) : a.setTransform(0, d, 0), a.setTransition(2 * (a.times.end - a.times.start)),
					a.updateActiveSlide(d)), (!b.freeModeFluid || 300 <= a.times.end - a.times.start) && a.updateActiveSlide(a.positions
					.current)) : (x = 0 > c ? "toNext" : "toPrev", "toNext" == x && 300 >= a.times.end - a.times.start && (30 > d ||
					!b.shortSwipes ? a.swipeReset() : a.swipeNext(!0)), "toPrev" == x && 300 >= a.times.end - a.times.start && (30 >
					d || !b.shortSwipes ? a.swipeReset() : a.swipePrev(!0)), e = p * b.slidesPerGroup, "toNext" == x && 300 < a.times
				.end - a.times.start && (d >= 0.5 * e ? a.swipeNext(!0) : a.swipeReset()), "toPrev" == x && 300 < a.times.end - a
				.times.start && (d >= 0.5 * e ? a.swipePrev(!0) : a.swipeReset()));
	...
	window.addEventListener || (window.Element || (Element = function() {}), Element.prototype.addEventListener =
		...
		}, Element.prototype.removeEventListener = HTMLDocument.prototype.removeEventListener = removeEventListener =
		function(a, b) {
	...
	if (f.nodeType || document.querySelectorAll && 0 != document.querySelectorAll(f).length) {
		...
		for (m = a.container.childNodes.length - 1; 0 <= m; m--)
			if (a.container.childNodes[m].className)
				for (var q = a.container.childNodes[m].className.split(" "), F = 0; F < q.length; F++) q[F] === b.wrapperClass &&
					(n = a.container.childNodes[m]);
		a.wrapper = n;
		h = "horizontal" == b.mode;
		a.touchEvents = {
		...
		a._calcSlides = function() {
			...
			for (var d = 0; d < a.wrapper.childNodes.length; d++)
				if (a.wrapper.childNodes[d].className)
					for (var e = a.wrapper.childNodes[d].className.split(" "), f = 0; f < e.length; f++) e[f] === b.slideClass && a.slides
						.push(a.wrapper.childNodes[d]);
		...
		a.ie10 && !b.onlyExternal && (h ? a.wrapper.classList.add("swiper-wp8-horizontal") : a.wrapper.classList.add(
			"swiper-wp8-vertical"));
		...
		a.init = function(c) {
			var d = window.getComputedStyle(a.container, null).getPropertyValue("width"),
				...
				f = parseInt(d, 10),
				g = parseInt(e, 10);
			if (isNaN(f) || a.ie8 && 0 < d.indexOf("%")) f = a.container.offsetWidth - parseInt(window.getComputedStyle(a.container,
				null).getPropertyValue("padding-left"), 10) - parseInt(window.getComputedStyle(a.container, null).getPropertyValue(
				"padding-right"), 10);
			if (isNaN(g) || a.ie8 && 0 < e.indexOf("%")) g = a.container.offsetHeight - parseInt(window.getComputedStyle(a.container,
				null).getPropertyValue("padding-top"), 10) - parseInt(window.getComputedStyle(a.container, null).getPropertyValue(
				"padding-bottom"), 10);
			if (c || !(a.width == f && a.height == g)) {
				...
				k = a.slides.length;
				b.scrollContainer ? (h || (n.style.width = "", n.style.height = "", a.slides[0].style.width = "", a.slides[0].style
						.height = ""), c = a.slides[0].offsetWidth, d = a.slides[0].offsetHeight, l = h ? a.width : a.height, p = h ?
					c : d, e = c, f = d) : (z ? (c = a.slides[0].offsetWidth, d = a.slides[0].offsetHeight) : (c = a.width / c, d =
						a.height / d), l = h ? a.width : a.height, p = h ? c : d, e = h ? k * c : a.width, f = h ? a.height : k * d, z &&
					(b.slidesPerSlide = Math.round(l / p)));
				y = h ? e : f;
				...
				n.style.width = e + "px";
				n.style.height = f + "px";
				0 < b.initialSlide && (b.initialSlide < k && !B) && (a.realIndex = a.activeSlide = b.initialSlide, b.loop && (a.activeSlide =
					a.realIndex - b.slidesPerSlide), h ? (a.positions.current = -b.initialSlide * c, a.setTransform(a.positions.current,
					0, 0)) : (a.positions.current = -b.initialSlide * d, a.setTransform(0, a.positions.current, 0)));
		...
		a.updatePagination = function() {
			if (!(2 > a.slides.length)) {
				...
				if (c) {
					...
					for (var c = document.querySelectorAll(b.pagination + " ." + b.paginationClass).length, e = b.loop ? a.realIndex -
							b.slidesPerSlide : a.realIndex, f = e + (b.slidesPerSlide - 1), d = e; d <= f; d++) {
						var g = d;
						g >= c && (g -= c);
						0 > g && (g = c + g);
						g < k && (document.querySelectorAll(b.pagination + " ." + b.paginationClass).item(g).className = b.paginationClass +
							" " + b.paginationActiveClass);
						d == e && (document.querySelectorAll(b.pagination + " ." + b.paginationClass).item(g).className +=
							" swiper-activeslide-switch")
					}
				}
			}
		};
		a.createPagination = function() {
			if (b.pagination && b.createPagination) {
				for (var c = "", d = a.slides.length, d = b.loop ? d - 2 * b.slidesPerSlide : d, e = 0; e < d; e++) c +=
					'<span class="' + b.paginationClass + '"></span>';
		...
		a.ie10 ? (n.addEventListener(a.touchEvents.touchStart, s, !1), document.addEventListener(a.touchEvents.touchMove, v,
			!1), document.addEventListener(a.touchEvents.touchEnd, r, !1)) : (a.support.touch && (n.addEventListener(
				"touchstart", s, !1), n.addEventListener("touchmove", v, !1), n.addEventListener("touchend", r, !1)), b.simulateTouch &&
			(n.addEventListener("mousedown", s, !1), document.addEventListener("mousemove", v, !1), document.addEventListener(
				"mouseup", r, !1)));
		a.destroy = function(c) {
			(!1 === c ? c : 1) && window.removeEventListener(a.resizeEvent, a.resizeFix, !1);
			a.ie10 ? (n.removeEventListener(a.touchEvents.touchStart, s, !1), document.removeEventListener(a.touchEvents.touchMove,
				v, !1), document.removeEventListener(a.touchEvents.touchEnd, r, !1)) : (a.support.touch && (n.removeEventListener(
				"touchstart", s, !1), n.removeEventListener("touchmove", v, !1), n.removeEventListener("touchend", r, !1)), n.removeEventListener(
				"mousedown", s, !1), document.removeEventListener("mousemove", v, !1), document.removeEventListener("mouseup",
				r, !1));
		...
		if (b.keyboardControl) {
			var G = function(b) {
				...
				h ? ((37 == d || 39 == d) && b.preventDefault(), 39 == d && a.swipeNext(), 37 == d && a.swipePrev()) : ((38 == d ||
					40 == d) && b.preventDefault(), 40 == d && a.swipeNext(), 38 == d && a.swipePrev())
			};
		...
		if (b.mousewheelControl) {
			...
			var H = function(c) {
				...
				var d = a._wheelEvent,
					e;
				c.detail ? e = -c.detail : "mousewheel" == d ? e = c.wheelDelta : "DOMMouseScroll" == d ? e = -c.detail : "wheel" ==
					d && (e = Math.abs(c.deltaX) > Math.abs(c.deltaY) ? -c.deltaX : -c.deltaY);
				b.freeMode ? (h ? a.getTranslate("x") : a.getTranslate("y"), h ? (d = a.getTranslate("x") + e, e = a.getTranslate(
					"y"), 0 < d && (d = 0), d < -g() && (d = -g())) : (d = a.getTranslate("x"), e = a.getTranslate("y") + e, 0 < e &&
					(e = 0), e < -g() && (e = -g())), a.setTransition(0), a.setTransform(d, e, 0)) : 0 > e ? a.swipeNext() : a.swipePrev();
				c.preventDefault();
				return !1
			};
		...
		a.swipeNext = function(c) {
			...
			c = Math.abs(c);
			if (d != y && (!(c >= g()) || b.loop)) return d > g() && !b.loop && (d = g()), b.loop && d >= g() + l && (d = g() +
					l), h ? a.setTransform(-d, 0, 0) : a.setTransform(0, -d, 0), a.setTransition(b.speed), a.updateActiveSlide(-d),
				E(), !0
		};
		...
		a.swipeTo = function(c, d, e) {
			...
			if (!(c > k - 1) && (!(0 > c) || b.loop)) return e = !1 === e ? !1 : e || !0, d = 0 === d ? d : d || b.speed, b.loop &&
				(c += b.slidesPerSlide), c > k - b.slidesPerSlide && (c = k - b.slidesPerSlide), c = -c * p, I && (b.loop && 0 <
					b.initialSlide && b.initialSlide < k) && (c -= b.initialSlide * p, I = !1), h ? a.setTransform(c, 0, 0) : a.setTransform(
					0, c, 0), a.setTransition(d), a.updateActiveSlide(c), e && E(), !0
		};
		a.updateActiveSlide = function(c) {
			a.previousSlide = a.realIndex;
			a.realIndex = Math.round(-c / p);
			b.loop ? (a.activeSlide = a.realIndex - b.slidesPerSlide, a.activeSlide >= k - 2 * b.slidesPerSlide && (a.activeSlide =
					k - 2 * b.slidesPerSlide - a.activeSlide), 0 > a.activeSlide && (a.activeSlide = k - 2 * b.slidesPerSlide + a.activeSlide)) :
				a.activeSlide = a.realIndex;
...
Swiper.prototype = {
	...
	getTranslate: function(f) {
		var b = this.wrapper,
			g, j;
		g = (window.WebKitCSSMatrix ? new WebKitCSSMatrix(window.getComputedStyle(b, null).webkitTransform) : window.getComputedStyle(
				b, null).MozTransform || window.getComputedStyle(b, null).OTransform || window.getComputedStyle(b, null).MsTransform ||
			window.getComputedStyle(b, null).msTransform || window.getComputedStyle(b, null).transform || window.getComputedStyle(
				b, null).getPropertyValue("transform").replace("translate(", "matrix(1, 0, 0, 1,")).toString().split(",");
		this.params.useCSS3Transforms ? ("x" == f && (j = 16 == g.length ? parseInt(g[12], 10) : parseInt(g[4], 10)), "y" ==
			f && (j = 16 == g.length ? parseInt(g[13], 10) : parseInt(g[5], 10))) : ("x" == f && (j = parseInt(b.style.left,
			10) || 0), "y" == f && (j = parseInt(b.style.top, 10) || 0));
		return j
	},
	setTransform: function(f, b, g) {
		...
		b = b || 0;
		g = g || 0;
		this.params.useCSS3Transforms ? this.support.threeD ? j.webkitTransform = j.MsTransform = j.msTransform = j.MozTransform =
			j.OTransform = j.transform = "translate3d(" + f + "px, " + b + "px, " + g + "px)" : (j.webkitTransform = j.MsTransform =
				j.msTransform = j.MozTransform = j.OTransform = j.transform = "translate(" + f + "px, " + b + "px)", this.ie8 &&
				(j.left = f + "px", j.top = b + "px")) : (j.left = f + "px", j.top = b + "px");
